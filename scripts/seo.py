...
SPORT_NAV_ITEMS = {
    "nfl": (("my_team", "My Team"), ("decision", "Decision"), ("rankings", "Rankings"),
...
(function(){
  ...
  var sitePlayer = bar.querySelector('[data-player-search], #site-player-search');
  var siteList = sitePlayer && document.getElementById(sitePlayer.getAttribute('list'));
  if (sitePlayer && siteList){
    var loaded = siteList.options.length > 0;
    var loading = null;
    function loadPlayers(){
      if (loaded || !sitePlayer.dataset.index) return Promise.resolve();
      if (loading) return loading;
      loading = fetch(sitePlayer.dataset.index, {credentials:'same-origin'})
        .then(function(response){ if (!response.ok) throw new Error(); return response.json(); })
        .then(function(data){
          (data.players || []).forEach(function(player){
            var option = document.createElement('option');
            option.value = player.name + ' · ' + player.team + ' ' + player.position;
            option.dataset.url = player.url;
    ...
    function findPlayer(allowPartial){
      var query = normalized(sitePlayer.value);
      var options = Array.prototype.slice.call(siteList.options || []);
      var exact = options.find(function(option){
        var value = normalized(option.value), name = value.split(' · ')[0];
        return value === query || name === query;
      });
      if (exact || !allowPartial || !query) return exact;
      return options.find(function(option){
        var value = normalized(option.value), name = value.split(' · ')[0];
        return name.indexOf(query) === 0;
      }) || options.find(function(option){ return normalized(option.value).indexOf(query) >= 0; });
    }
    function openPlayer(option){
      if (!option) return false;
      if (option.dataset.url) location.href = option.dataset.url;
      else if (option.dataset.id)
        location.href = '/decision-room/nfl/?a=' + encodeURIComponent(option.dataset.id);
      else return false;
      return true;
    }
    sitePlayer.addEventListener('focus', loadPlayers);
    sitePlayer.addEventListener('input', loadPlayers);
    sitePlayer.addEventListener('change', function(){
      loadPlayers().then(function(){ openPlayer(findPlayer(false)); });
    });
    sitePlayer.addEventListener('keydown', function(event){
      if (event.key !== 'Enter') return;
      event.preventDefault();
      loadPlayers().then(function(){ openPlayer(findPlayer(true)); });
    });
  }
...
GLOBAL_FOOTER = """<footer class="global-footer"><div class="wrap">
  ...
  <div class="frow"><div class="fcol"><h3>Decision tools</h3><p><a href="/my-team/">My Team</a><br><a href="/decision-room/nfl/">NFL Decision Room</a><br><a href="/decision-room/college/">College Decision Room</a><br><a href="/nfl/data/">NFL Fantasy Data</a><br><a href="/nfl/who-should-i-draft/">Advanced Draft Comparison</a></p></div>
