...
def build_page(canonical: dict, summary: dict) -> str:
    ...
    <script>(function(){
      ...
      function providerName(){return state.capture&&state.capture.provider==='yahoo'?'Yahoo':'ESPN';}
      ...
      function render(record){
        ...
        managerReview.classList.remove('is-complete');managerStep.textContent='One quick step';managerTitle.textContent='Match managers';managerCopy.textContent=providerName()+' found accounts with similar names. Tell us whether each pair belongs to the same person.';reviewManagers.hidden=true;setupReady.hidden=true;
      ...
      clear.addEventListener('click',function(){if(state.capture&&state.capture.provider==='yahoo'){try{localStorage.removeItem('lineupBeatYahooHistoryV1');}catch(_){}window.postMessage({type:'LB_LEAGUE_HISTORY_CLEAR_COMPLETE',version:1},location.origin);}else{window.postMessage({type:'LB_LEAGUE_HISTORY_CLEAR_REQUEST',version:1},location.origin);}});
      ...
      reviewManagers.addEventListener('click',function(){managerReview.classList.remove('is-complete');managerStep.textContent='One quick step';managerTitle.textContent='Match managers';managerCopy.textContent=providerName()+' found accounts with similar names. Tell us whether each pair belongs to the same person.';reviewManagers.hidden=true;setupReady.hidden=true;if(allAnswered())showComplete();else showPair(0);});
    ...
    <meta name="description" content="Import, review, and share your ESPN or Yahoo fantasy football league history with LineupBeat.">
