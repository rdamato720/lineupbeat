...
def build_page(canonical: dict, summary: dict) -> str:
    ...
    @media(max-width:760px){.lh-head{grid-template-columns:1fr}.lh-meta{justify-content:space-between}.import-line{align-items:flex-start;flex-direction:column}.match-people{grid-template-columns:1fr}.match-or{text-align:center}.choice-actions{grid-template-columns:1fr}.review-footer{align-items:flex-start;flex-direction:column}.dashboard,.source-grid{grid-template-columns:1fr}.record-grid,.manager-grid{grid-template-columns:1fr 1fr}.season-grid{grid-template-columns:1fr}.tabs{top:3.4rem}}
    ...
    <script>(function(){
      ...
      window.addEventListener('message',function(event){
        ...
        if(event.data.type==='LB_LEAGUE_HISTORY_REVIEW_COMPLETE'){if(event.data.ok){state.dirty=false;updateSave();result.textContent='Manager matches saved. Next up: team history.';}else result.textContent='Manager matches could not be saved.';}
