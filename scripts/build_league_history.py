...
def build_page(canonical: dict, summary: dict) -> str:
    ...
    .setup-layout{display:grid;grid-template-columns:13.5rem minmax(0,1fr);min-height:28rem}.setup-workspace{min-width:0;padding:1.5rem 1.65rem}.setup-rail{display:flex;flex-direction:column;border-right:1px solid var(--history-line);padding:1.55rem 1.25rem;background:#0b100e}.setup-rail-kicker{color:var(--signal);font:800 .7rem var(--agate);letter-spacing:.1em;text-transform:uppercase}.setup-rail ol{display:grid;gap:.95rem;margin:1.35rem 0;padding:0;list-style:none}.setup-rail li{display:grid;grid-template-columns:2rem 1fr;gap:.65rem;align-items:center;color:#727b75;font:700 .8rem/1.25 var(--agate)}.setup-rail li b{display:grid;place-items:center;width:1.8rem;height:1.8rem;border:1px solid #3a4540;border-radius:50%;font:800 .72rem var(--data)}.setup-rail li.current{color:var(--ink)}.setup-rail li.current b{border-color:var(--signal);background:var(--signal);color:#07100b}.setup-rail li.complete{color:var(--history-muted)}.setup-rail li.complete b{border-color:#637734;color:var(--signal)}.setup-rail>small{margin-top:auto;color:#7e8981;font:700 .67rem/1.35 var(--agate);letter-spacing:.07em;text-transform:uppercase}.setup-heading{margin:1.4rem 0 1rem}.setup-heading span,.connection-heading span{color:var(--signal);font:800 .7rem var(--agate);letter-spacing:.1em;text-transform:uppercase}.setup-heading h2{margin:.35rem 0 0;font:700 clamp(1.7rem,3vw,2.35rem)/1 var(--display);letter-spacing:-.025em}.setup-heading p{margin:.45rem 0 0;color:var(--history-muted);font:400 .92rem/1.45 var(--agate)}.source-tabs{display:grid;width:auto;grid-template-columns:repeat(3,minmax(0,1fr));gap:.7rem;border:0}.source-tabs button{display:grid;grid-template-rows:2.6rem auto auto;gap:.45rem;min-width:0;min-height:9.5rem;border:1px solid var(--history-line);padding:1rem;background:#0b100e;color:var(--ink);text-align:left;text-transform:none}.source-tabs button:last-child{border-right:1px solid var(--history-line)}.source-tabs button:hover,.source-tabs button:focus-visible{border-color:#6d7b73;background:#111815}.source-tabs button[aria-selected=true]{border-color:var(--signal);background:linear-gradient(145deg,#152016,#0d130f);color:var(--ink);box-shadow:inset 0 3px 0 var(--signal)}.source-tabs button strong{font:750 1rem/1.15 var(--agate)}.source-tabs button small{color:var(--history-muted);font:600 .72rem/1.3 var(--agate)}.provider-mark{display:flex;align-items:center;color:var(--signal);font:900 1.45rem/1 var(--data);letter-spacing:-.06em}.connection-stage{margin-top:1rem;border:1px solid var(--history-line);background:#0b100e;padding:1rem}.connection-stage[hidden]{display:none}.connection-heading{margin-bottom:.75rem}.connection-heading h3{margin:.3rem 0 0;font:750 1.2rem/1.1 var(--agate)}.source-panel{margin-top:0}.source-panel>div:first-child>strong{display:block;margin-bottom:.25rem;font:750 1rem var(--agate)}.source-setup{margin-top:1.1rem}.import-summary{padding:1.3rem 0 0}.has-import .setup-layout{grid-template-columns:13.5rem minmax(0,1fr)}.has-import .import-card{max-width:64rem;padding:0}.has-import .source-setup{display:none}.history-ready .import-card{padding:0}.history-ready .setup-workspace{padding:.75rem 1rem}.history-ready .setup-rail{display:none}.history-ready .setup-layout{display:block;min-height:0}
    ...
    .history-empty .lh-head{min-height:0;padding:1.55rem 1.5rem}.history-empty .lh-head h1{max-width:none;font-size:clamp(2.3rem,5vw,3.8rem)}.history-empty .lh-meta{display:none}
    ...
    .publish-panel[hidden],.publish-result[hidden],.restore-publication[hidden]{display:none}.publish-panel{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:1rem 2rem;align-items:start;margin:.75rem 0 0;padding:1rem 1.1rem;border:1px solid var(--history-line);background:radial-gradient(circle at 85% 0,#c6f53c12,transparent 18rem),var(--history-panel-2)}
    ...
    <script>(function(){
      ...
      var setupSteps=[].slice.call(document.querySelectorAll('[data-setup-step]'));
      ...
      function render(record){
        ...
        managerReview.classList.remove('is-complete');managerStep.textContent='Step 3 of 4';managerTitle.textContent='Match managers';managerCopy.textContent=providerName()+' found accounts with similar names. Tell us whether each pair belongs to the same person.';reviewManagers.hidden=true;setupReady.hidden=true;
      ...
      reviewManagers.addEventListener('click',function(){managerReview.classList.remove('is-complete');managerStep.textContent='Step 3 of 4';managerTitle.textContent='Match managers';managerCopy.textContent=providerName()+' found accounts with similar names. Tell us whether each pair belongs to the same person.';reviewManagers.hidden=true;setupReady.hidden=true;setSetupStep(3);if(allAnswered())showComplete();else showPair(0);});
    ...
    <main class="lh"><div class="lh-status"><i></i>private by default</div>
      ...
      <section class="import-card" aria-labelledby="import-title"><div class="setup-layout"><aside class="setup-rail" aria-label="League setup progress"><span class="setup-rail-kicker">My League setup</span><ol><li class="current" data-setup-step="1"><b>1</b><span>Choose platform</span></li><li data-setup-step="2"><b>2</b><span>Connect and import</span></li><li data-setup-step="3"><b>3</b><span>Match managers</span></li><li data-setup-step="4"><b>4</b><span>View and share</span></li></ol><small>Private until you publish.</small></aside><div class="setup-workspace"><div class="import-line"><div class="import-copy"><strong id="import-title">Set up your league history</strong><span id="import-status" role="status">Choose where your league is hosted.</span></div><div class="import-actions"><button id="edit-manager-matches" class="quiet" type="button" hidden>Manager matches</button><button id="clear-import" class="quiet" type="button" hidden>Remove import</button><button id="retry-shared-league" type="button" hidden>Try again</button></div></div><div class="source-setup"><div class="setup-heading"><span>Step 1 of 4</span><h2>Choose your fantasy platform.</h2><p>We only show services LineupBeat can connect today.</p></div><div class="source-tabs" role="tablist" aria-label="League provider"><button type="button" role="tab" data-history-source="espn" aria-selected="false"><span class="provider-mark">ESPN</span><strong>ESPN Fantasy</strong><small>Automatic history import</small></button><button type="button" role="tab" data-history-source="yahoo" aria-selected="false"><span class="provider-mark">Y!</span><strong>Yahoo Fantasy</strong><small>Read-only account access</small></button><button type="button" role="tab" data-history-source="cbs" aria-selected="false"><span class="provider-mark">CBS</span><strong>CBS Fantasy</strong><small>Add one visible season at a time</small></button></div><div class="connection-stage" id="connection-stage" hidden><div class="connection-heading"><span>Step 2 of 4</span><h3 id="connection-title">Connect your league.</h3></div><div class="source-panel" data-source-panel="espn" hidden><div><strong>Import from ESPN</strong><p>Install the LineupBeat connector, open any ESPN league page, and import up to 25 available seasons.</p></div><div class="import-actions"><a id="install-connector" href="/my-team/extension/">Install connector</a><button id="check-extension" class="quiet" type="button">Check ESPN</button></div></div><div class="source-panel" data-source-panel="yahoo" hidden><div><strong>Import from Yahoo</strong><p>Authorize read-only access, choose your league, and import its connected seasons.</p></div><div class="import-actions"><button id="connect-yahoo" type="button">Connect Yahoo</button><button id="disconnect-yahoo" class="quiet" type="button" hidden>Disconnect</button></div><div class="yahoo-picker" id="yahoo-picker" hidden><label>League history<select id="yahoo-family"></select></label><div class="import-actions"><button id="import-yahoo-history" type="button">Import history</button></div><p id="yahoo-progress" role="status" hidden></p></div></div><div class="source-panel" data-source-panel="cbs" hidden><div><strong>Import from CBS</strong><p>Install the connector, open My League → History on CBS, and add the visible season. Repeat for each season you want to preserve.</p></div><div class="import-actions"><a href="/my-team/extension/">Install connector</a><button id="check-cbs" class="quiet" type="button">Check CBS</button></div></div></div></div>
        <div class="import-summary" id="import-summary"><div class="capture-stats" id="capture-stats" hidden></div><section class="manager-review" id="manager-review"><div class="review-head"><div><span class="review-step" id="manager-step">Step 3 of 4</span><h2 id="manager-title">Match managers</h2><p id="manager-copy">Your provider found accounts with similar names. Tell us whether each pair belongs to the same person.</p></div><button class="quiet" id="review-managers" type="button" hidden>Review</button></div><div class="match-flow" id="match-flow" hidden><section class="match-card" id="match-card"><div class="match-progress"><span id="match-progress">Match 1</span><span>Possible duplicate</span></div><div class="match-people"><article class="person-card"><small>Provider account A</small><strong id="person-a-name"></strong><div class="person-meta" id="person-a-meta"></div><details class="person-aliases"><summary id="person-a-teams-summary"></summary><p id="person-a-teams"></p></details></article><span class="match-or">and</span><article class="person-card"><small>Provider account B</small><strong id="person-b-name"></strong><div class="person-meta" id="person-b-meta"></div><details class="person-aliases"><summary id="person-b-teams-summary"></summary><p id="person-b-teams"></p></details></article></div><p class="match-question">Are these the same person?</p><div class="choice-actions"><button id="same-person" type="button" aria-pressed="false">Yes, same person</button><button id="different-people" type="button" aria-pressed="false">No, different people</button></div></section><section class="match-complete" id="match-complete" hidden><h3 id="complete-title">Manager matches complete</h3><p id="complete-copy">You can change any answer before saving.</p><div class="decision-summary" id="decision-summary"></div><div class="import-actions"><button class="quiet" id="change-answers" type="button">Change answers</button></div></section></div><div class="review-footer"><p id="other-manager-count">Other managers already look distinct.</p><div class="import-actions"><button id="save-manager-matches" type="button" disabled>Save manager matches</button></div></div><p class="review-result" id="review-result" role="status"></p></section><section class="setup-ready" id="setup-ready" hidden><small>Step 4 of 4</small><h2>Your league history is ready</h2><p>Every historical team and season is included automatically. Manager matches are saved on this device.</p></section></div></div></div></section>
      ...
      <section class="panel active" id="overview"><div class="dashboard">
        ...
        <div class="notice"><strong>Private by default. </strong>Your imported archive is calculated in this browser.</div></section>
    ...
    </main>{seo.site_footer()}{script}<script src="/assets/league-history-dashboard.js"></script><script src="/assets/yahoo-history.js"></script></body></html>'''
