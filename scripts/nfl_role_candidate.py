"""Research-only usage candidate, not imported by production builders.

This frozen fork of nfl-usage-v2 tests transfer-role weighting and the current
quarterback's full-participation passing volume/TD rate. Its 2025 regression
replay failed the predefined deployment gate. Keep it isolated from production.

No projection workbook, expert ranking, ADP, news text, or paid provider is an
input. Model choices are stored separately with their evaluation.
"""
...
class History:
    ...
    def features(self, season, week, *, current=False, asof=None):
        ...
        for p in roster:
            ...
            prior_role = [r for r in hist if r['season'] < season][-12:]
            item['same_team_prior_n'] = sum(r['team'] == p['team'] for r in prior_role)
            # Full-participation QB games separate a starter's passing usage
            # from relief appearances. Missing snaps cannot qualify a game.
            full_games = [r for r in hist if r.get('offense_pct',0) >= .8
                          and r['attempts'] > 0][-12:] if p['position'] == 'QB' else []
            item['qb_full_games'] = {
                'n':len(full_games),
                'attempts':weighted_average([r['attempts'] for r in full_games]),
                'total_attempts':sum(r['attempts'] for r in full_games),
                'passing_tds':sum(r['passing_tds'] for r in full_games),
            }
            shares = {'prior':{},'recent':{}}
...
def predict(features, config):
    ...
    for club,t in teams.items():
        ...
        full = starters.get(club,{}).get('qb_full_games',{})
        if full.get('n'):
            weight = config.get('qb_volume_weight',0)*full['n']/(full['n']+3)
            starter_attempts = full['attempts']/.98
            attempts = clamp((1-weight)*attempts+weight*starter_attempts,plays*.35,plays*.75)
        ...
        for field,opportunity in (('passing_tds','attempts'),('rushing_tds','carries')):
            ...
            if field == 'passing_tds' and full.get('total_attempts'):
                strength = config['efficiency_prior']*2
                qb_rate = (full['passing_tds']+strength*league_rate)/(full['total_attempts']+strength)
                weight = config.get('qb_td_weight',0)
                rate = (1-weight)*rate+weight*qb_rate
    ...
    for p in features['players']:
        ...
        for key in ('carries','targets','goal_line_carries','red_zone_targets','end_zone_targets'):
            ...
            prior_strength = config['player_prior_games']
            # A prior workload from another club is weaker evidence of this
            # team's role. The factor is selected on historical transfers,
            # never inferred from an expert's current projection.
            if p['prior_n'] and p.get('same_team_prior_n') == 0 and p['recent_n']:
                prior_strength *= config.get('transfer_prior_factor',1)
            share=blend(prior,recent,p['recent_n'],prior_strength)
        ...
        row={'id':p['id'],'name':p['name'],'position':pos,'team':club,'stat_projection':stat,
             ...
             'evidence':{'prior_appearances':p['prior_n'],'current_appearances':p['recent_n'],
                         ...
                         'same_team_prior_appearances':p.get('same_team_prior_n'),
                         'qb_full_participation_games':p.get('qb_full_games',{}).get('n',0),
