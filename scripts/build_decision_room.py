...
def render_home(payload: dict, college_payload: dict) -> str:
    ...
    calls = [r for r in closest_calls(players, "half_ppr", limit=20) if not r["is_tie"]][:2]
    values, fades = value_signals(players, "half_ppr")
    movers = scoring_movers(players)[:2]
...
<main id="lineup-beat-home" class="hp-shell">
  ...
  <section class="hp-section"><div class="hp-section-head"><small>Choose your context</small><h2>NFL or College</h2><p>NFL uses validated 2026 full-season projections. College provides separate Week 1 Yahoo projections and season projections with different player pools.</p></div><div class="hp-actions hp-nfl-actions"><a href="{NFL_ROOM_PATH}"><b>Compare</b><span>Open the NFL Decision Room</span></a><a href="/nfl/rankings/"><b>Rankings</b><span>Browse the current NFL order</span></a><a href="/nfl/projections/"><b>Projections</b><span>Inspect validated season inputs</span></a><a href="/nfl/data/"><b>Fantasy Data</b><span>Draft value, ADP, coaching context, and advanced tools</span></a></div></section>
  <section class="hp-section hp-board"><div class="hp-section-head"><small>Validated signals</small><h2>Today’s Decision Board</h2><p>NFL full-season projections · Half-PPR unless noted</p></div><div class="hp-board-grid">{''.join(board_call(r) for r in calls)}{''.join(board_signal(r, 'Value') for r in values[:1])}{''.join(board_signal(r, 'Fade') for r in fades[:1])}{''.join(board_mover(r) for r in movers)}</div></section>
  <section class="hp-section hp-method"><div class="hp-section-head"><small>Methodology &amp; accountability</small><h2>The call should show its work.</h2><p>Lineup Beat separates validated inputs from recommendations, shows the boundary that changes a pick, and is building a timestamped record so decisions cannot be silently rewritten.</p></div><div class="hp-actions"><a href="/about/"><b>How decisions work</b><span>Inputs, uncertainty, and accountability</span></a><a href="{COLLEGE_ROOM_PATH}"><b>College Week 1</b><span>2,205 players · 64 teams · Yahoo only</span></a><a href="/nfl/who-should-i-draft/"><b>Advanced comparison</b><span>216-player pool with validated historical weekly context</span></a></div></section>
