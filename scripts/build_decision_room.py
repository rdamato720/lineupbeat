...
function adpMissingText(a,b){let x=adpState(a,b);if(x.state==='present')return'';if(x.state==='one-missing')return `Unavailable — validated ADP is unavailable for ${safe(x.missing[0].name)}.`;return `Unavailable — validated ADP is unavailable for both ${safe(a.name)} and ${safe(b.name)}.`}
...
function agreementText(a,b,x){let clauses=[a,b].filter(p=>x.groups[p.id].length).map(p=>`${joinWords(x.groups[p.id].map(s=>s.category))} ${x.groups[p.id].length===1?'favors':'favor'} ${safe(p.name)}`);return `${clauses.join('; ')}. Evidence agreement is ${x.state}.`}
