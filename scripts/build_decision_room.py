...
def render_home(payload: dict, college_payload: dict) -> str:
    ...
    icons = {
        "rankings": '<svg class="hp-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 19V11M12 19V5M19 19V8"/></svg>',
        "projections": '<svg class="hp-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m4 17 5-5 4 3 7-9"/><path d="M16 6h4v4"/></svg>',
        "draft": '<svg class="hp-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="7"/><path d="M12 2v4M12 18v4M2 12h4M18 12h4"/></svg>',
        "college": '<svg class="hp-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m3 9 9-5 9 5-9 5-9-5Z"/><path d="M7 12v5c3 2 7 2 10 0v-5M21 9v6"/></svg>',
        "team": '<svg class="hp-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><circle cx="17" cy="10" r="2"/><path d="M3 20c0-4 2-6 6-6s6 2 6 6M15 15c4 0 6 2 6 5"/></svg>',
        "league": '<svg class="hp-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 8v5l3 2"/><path d="M5 5a9 9 0 1 1-2 6"/><path d="M3 4v-5h5"/></svg>',
        "decision": '<svg class="hp-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 6h5l3 5 3-5h5M4 18h5l3-5 3 5h5"/></svg>',
    }
    hero = f'''<section class="hp-home-hero"><div class="hp-wordmark-field" aria-hidden="true"><span>LINEUPBEAT LINEUPBEAT LINEUPBEAT</span><span>LINEUPBEAT LINEUPBEAT LINEUPBEAT</span><span>LINEUPBEAT LINEUPBEAT LINEUPBEAT</span><span>LINEUPBEAT LINEUPBEAT LINEUPBEAT</span></div><div class="hp-hero-grid"><div class="hp-home-copy"><div class="lb-eyebrow">NFL + COLLEGE FANTASY FOOTBALL</div><h1>YOUR SEASON.<br><span>IN FOCUS.</span></h1><p>Independent rankings, projections, roster tools, league history, and decision support—all in one place.</p><div class="lb-hero-actions"><a class="lb-btn lb-btn-primary" href="/nfl/rankings/">Explore NFL <b>→</b></a><a class="lb-btn lb-btn-secondary" href="/college-fantasy-football/week-1/">Explore College <b>→</b></a></div></div><div class="hp-product-stage" aria-label="Explore LineupBeat products"><div class="hp-stage-main"><div class="hp-stage-top"><small>LINEUPBEAT</small><b>{payload['season']} NFL</b></div><h2>Built for every part of your season.</h2><div class="hp-stage-rows"><a href="/nfl/rankings/"><i>01</i>{icons['rankings']}<span>NFL Rankings</span><b>→</b></a><a href="/nfl/projections/"><i>02</i>{icons['projections']}<span>Projections</span><b>→</b></a><a href="/nfl/who-should-i-draft/"><i>03</i>{icons['draft']}<span>Draft Tools</span><b>→</b></a></div></div><a class="hp-stage-card hp-stage-college" href="/college-fantasy-football/week-1/">{icons['college']}<small>COLLEGE</small><strong>{len(college_payload['players']):,}</strong><span>Week 1 players →</span></a><a class="hp-stage-card hp-stage-team" href="/my-team/">{icons['team']}<small>MY TEAM</small><strong>YOUR ROSTER</strong><span>Private analysis →</span></a><a class="hp-stage-card hp-stage-league" href="/my-league/">{icons['league']}<small>MY LEAGUE</small><strong>THE HISTORY</strong><span>Build the record book →</span></a></div></div><aside class="hp-who"><small>WHO WE ARE</small><h2>Fantasy research without the noise.</h2><p>Our projections. Clear explanations. Honest limits.</p><a href="/about/">Meet LineupBeat →</a></aside></section>'''
    tools = f'''<section class="hp-section hp-platform" id="tools"><div class="hp-section-head"><small>EXPLORE LINEUPBEAT</small><h2>Everything your season needs.</h2></div><div class="hp-bento"><a class="hp-bento-card hp-bento-nfl" href="/nfl/rankings/"><span class="hp-icon-chip">{icons['rankings']}</span><small>NFL FANTASY</small><h3>Rankings.<br>Projections.<br>Draft tools.</h3><span>Explore NFL →</span></a><a class="hp-bento-card hp-bento-college" href="/college-fantasy-football/week-1/"><span class="hp-icon-chip">{icons['college']}</span><small>COLLEGE FANTASY</small><h3>Go deeper than the Saturday box score.</h3><span>Explore College →</span></a><a class="hp-bento-card hp-bento-team" href="/my-team/"><span class="hp-icon-chip">{icons['team']}</span><small>MY TEAM</small><h3>See your roster clearly.</h3><span>Analyze my team →</span></a><a class="hp-bento-card hp-bento-league" href="/my-league/"><span class="hp-icon-chip">{icons['league']}</span><small>MY LEAGUE</small><h3>Every season. Every champion.</h3><span>Build league history →</span></a></div></section>'''
    ...
    return f'''{START}{home_header()}<main id="lineup-beat-home" class="hp-shell">{hero}{tools}{decision}</main>{END}'''
...
<main id="decision-room" class="dr-shell" data-mode="weekly" data-season="2026" data-week="1">
  <section class="dr-hero">
    ...
    <p class="dr-lede">Choose two players. We’ll show the model pick first and the full evidence only when you want it.</p>
    <p class="dr-week-note">Week 1 · Half-PPR default · Current injuries are not included.</p>
    <section class="dr-compare" aria-labelledby="dr-compare-title">
      <div class="dr-compare-head"><div><small>MAKE A DECISION</small><h2 id="dr-compare-title">Choose two players</h2></div>
...
function draw(){let a=P[A.value],b=P[B.value],k=F.value;if(!a||!b||a.id===b.id){O.innerHTML='<p class="dr-error">Choose two different players.</p>';return}let ap=points(a,k),bp=points(b,k),gap=Math.abs(ap-bp),gp=pct(gap,Math.max(ap,bp)),lead=projectedLeader(a,b,k),cls=classification(gap,Math.max(ap,bp)),w=cls==='Toss-Up'?null:lead,r=w?(w.id===a.id?b:a):null,e=opinion(a,b),q=coverage(a,b,k,e),agree=agreement(a,b,k,e,cls,lead),qualified=!weekly||recommendationsAuthorized,call=cls==='Toss-Up'?'Too close to call':qualified?`Start ${safe(w.name)}`:`Model pick: ${safe(w.name)}`,projection=cls==='Toss-Up'?`No clear edge. The ${num(gap)}-point difference (${gp.toFixed(1)}%) is inside the deterministic no-call band.`:`${safe(w.name)} projects ${num(gap)} points (${gp.toFixed(1)}%) ahead of ${terminalName(r)}`,qualifier=agree.state==='Split'?'The supporting evidence is split.':agree.state==='Mixed'?'Some supporting evidence points the other way.':'',statusNote=!w?'No player clears our decision threshold.':qualified?'':'This model pick is not a validated lineup recommendation.';
O.innerHTML=`<section class="dr-verdict"><div><small>${qualified?'Lineup Beat call':'Projection-based answer'} · ${L[k]}</small><h2>${call}</h2><p><strong>${projection}</strong>${qualifier?` ${qualifier}`:''}</p><p class="dr-caution">Check current injury status before kickoff.${statusNote?` ${statusNote}`:''}</p></div><div class="dr-adv"><b>${gap?'+':''}${num(gap)}</b><span>Week 1 point difference</span></div></section><details class="dr-full"><summary>See full comparison <span>Players, evidence, and what could change the pick</span></summary><div class="dr-full-body">${playerCards(a,b,k)}<section class="dr-evidence"><div class="dr-evidence-title"><small>Evidence stack 02</small><h2>Why</h2></div><div class="dr-why-grid"><article><h3>Our Week 1 projection</h3><p>${cls} · ${num(gap)} points · ${gp.toFixed(1)}% difference.</p></article><article><h3>What the market says</h3><p>Unavailable — zero odds requests were made because the isolated credential is unavailable.</p></article><article><h3>Opponent matchup</h3><p>${safe(a.name)}: ${safe(a.matchup?.label)} factor ${Number(a.matchup?.projection_factor||1).toFixed(2)}. ${safe(b.name)}: ${safe(b.matchup?.label)} factor ${Number(b.matchup?.projection_factor||1).toFixed(2)}.</p></article><article><h3>Expected opportunity</h3><p>${safe(a.name)}: ${opportunity(a)}. ${safe(b.name)}: ${opportunity(b)}.</p></article><article><h3>Availability</h3><p>Current Week 1 injury reports are unavailable. The model covers QB, RB, WR, and TE only; no D/ST projection is included.</p></article><article><h3>Prior-year consistency</h3><p>${historyText(a,b,k)}</p></article></div></section><section class="dr-cases"><div class="dr-evidence-title"><small>Balanced evidence 03</small><h2>Case for each player</h2></div><div class="dr-case-grid"><article><h3>${safe(a.name)}</h3><ul>${caseFor(a,b,k,e)}</ul></article><article><h3>${safe(b.name)}</h3><ul>${caseFor(b,a,k,e)}</ul></article></div></section><section class="dr-boundary"><div class="dr-boundary-title"><small>Decision boundaries 04</small><h2>What changes the call</h2></div>${changeCards(a,b,k,lead,cls)}</section><section class="dr-quality"><div class="dr-evidence-title"><small>Transparency 05</small><h2>Data coverage and evidence agreement</h2><p><b>Data coverage</b> · ${q.present} of ${q.cats.length} evidence categories available for this pair.</p><p><b>Evidence agreement</b> · ${agree.state}. Classification describes projection-edge size, not probability or confidence.</p></div><div class="dr-quality-grid">${q.cats.map(x=>`<span class="${x[1].toLowerCase().replace(' ','-')}"><b>${x[0]}</b>${x[1]}</span>`).join('')}</div><p class="dr-stamp">Lineup Beat model ${safe(D.sources.model.updated_at)} · History: ${safe(D.sources.history.updated_at)} · Matchup: 2025 context · Market: unavailable${e?` · Editorial ${safe(e.evidence_date)} (historical/stale)`:''}</p></section></div></details>`}
function candidates(which){let other=which===A?B:A,base=D.players.filter(p=>p.id!==other.value);if(which===B&&!X.checked&&P[A.value])base=base.filter(p=>p.position===P[A.value].position);return base}
...
.dr-verdict{display:flex;justify-content:space-between;gap:2rem;align-items:center;padding:clamp(1.25rem,3vw,2rem);border:1px solid #52641f;border-top:5px solid var(--dr-lime);background:#131b0e}.dr-verdict small{color:var(--dr-lime);font-weight:800;letter-spacing:.08em;text-transform:uppercase}.dr-verdict h2{font-size:clamp(2.4rem,6vw,4.8rem);line-height:.95}.dr-verdict p{max-width:720px;margin:.7rem 0 0;color:#d8ddd8}.dr-verdict .dr-caution{color:#aeb7b0;font-size:.88rem}.dr-adv{text-align:center;min-width:150px}.dr-adv b{display:block;font:700 3rem var(--display);color:var(--dr-lime)}.dr-adv span{font:700 .67rem var(--agate);text-transform:uppercase;color:var(--dr-muted)}
.dr-full{margin-top:1rem;border:1px solid var(--dr-line);background:#0d1210}.dr-full>summary{display:flex;justify-content:space-between;gap:1rem;align-items:center;padding:1rem 1.2rem;color:var(--dr-lime);cursor:pointer;font:800 .82rem var(--agate);letter-spacing:.05em;text-transform:uppercase}.dr-full>summary span{color:var(--dr-muted);font-size:.7rem;letter-spacing:0;text-transform:none}.dr-full[open]>summary{border-bottom:1px solid var(--dr-line);background:#141b17}.dr-full-body{padding-top:.25rem}
...
.hp-who{position:relative;z-index:2;display:grid;grid-template-columns:140px minmax(0,1.2fr) minmax(260px,1fr) auto;gap:1.25rem;align-items:center;margin-top:2.5rem;padding:1.25rem 1.4rem;border:1px solid #3b473f;background:#0f1512}.hp-who small{color:var(--lime);font:800 .7rem var(--agate);letter-spacing:.12em}.hp-who h2{margin:0;font:700 clamp(1.3rem,2.4vw,1.8rem)/1.05 var(--display)}.hp-who p{margin:0;color:#bfc8c1;font:1rem/1.45 var(--agate)}.hp-who a{color:var(--lime);font:800 .72rem var(--agate);text-transform:uppercase;white-space:nowrap}.hp-dual-feature .hp-feature-players{min-height:220px;align-items:start}.hp-dual-feature .hp-feature-players>div{display:grid;grid-template-rows:130px auto auto auto;align-content:start}.hp-dual-feature .hp-feature-players>div>img,.hp-dual-feature .hp-college-feature .hp-feature-players>div>img{height:130px;padding:0}.hp-dual-feature .hp-college-feature .hp-feature-players>div>img{padding:18px}.hp-dual-feature .hp-feature>h2{min-height:2.05em;margin-bottom:1rem}.hp-decision-summary{padding:.9rem 1rem;border-left:4px solid var(--lime);background:#151c19}.hp-decision-summary small,.hp-evidence-grid span{display:block;color:var(--lime);font:800 .64rem var(--agate);letter-spacing:.08em;text-transform:uppercase}.hp-decision-summary strong{display:block;margin:.3rem 0;font:700 1.7rem var(--display)}.hp-decision-summary p{margin:0;color:#c8d0ca;font:.9rem/1.4 var(--agate)}.hp-evidence-grid{display:grid;grid-template-columns:1fr 1fr;min-height:176px;margin:1rem 0;border:1px solid #344039;background:#344039;gap:1px}.hp-evidence-grid>div{padding:.9rem;background:#101614}.hp-evidence-grid span{color:#8f9a92}.hp-evidence-grid b{display:block;margin-top:.45rem;color:#f2f5f0;font:700 .95rem/1.3 var(--agate)}.hp-dual-feature .hp-boundary{min-height:132px;margin-top:0}.hp-home-hero:before{display:none}
...
@media(max-width:900px){.hp-action-grid,.hp-signal-grid{grid-template-columns:1fr 1fr}.hp-call-grid,.hp-method-grid{grid-template-columns:1fr}.hp-call-art>.hp-identity{grid-template-columns:60px 1fr}.hp-call-art>.hp-identity .hp-id-art{width:60px}.hp-trust{grid-template-columns:1fr}.hp-who{grid-template-columns:1fr}.hp-dual-feature{grid-template-columns:1fr}.hp-dual-feature .hp-boundary{min-height:0}}
...
.hp-home-hero{min-height:760px;padding:clamp(3.2rem,6vw,5.5rem) max(1rem,calc((100% - 1240px)/2)) 2rem;background:radial-gradient(circle at 68% 34%,#22342a 0,transparent 31%),radial-gradient(circle at 90% 12%,#c6f53c17 0,transparent 24%),#050807}.hp-wordmark-field{position:absolute;z-index:-1;inset:0;overflow:hidden;opacity:.08;transform:rotate(-2deg) scale(1.06);pointer-events:none}.hp-wordmark-field span{display:block;white-space:nowrap;color:transparent;-webkit-text-stroke:1px #90a092;font:900 clamp(5rem,9vw,9rem)/1.04 var(--agate);letter-spacing:-.06em}.hp-hero-grid{position:relative;z-index:2;display:grid;grid-template-columns:minmax(360px,.82fr) minmax(520px,1.18fr);gap:clamp(2rem,5vw,5rem);align-items:center}.hp-home-copy h1{max-width:620px;margin:.7rem 0 1.25rem;color:#f7f9f5;font:950 clamp(4.2rem,7.3vw,7.1rem)/.82 var(--agate);letter-spacing:-.07em}.hp-home-copy h1 span{color:var(--lime)}.hp-home-copy>p{max-width:560px;color:#d1d8d2;font:1.12rem/1.55 var(--agate)}.hp-product-stage{position:relative;min-height:520px}.hp-stage-main{position:absolute;inset:36px 38px 54px 24px;padding:1.5rem;border:1px solid #607066;background:#0b110fdd;box-shadow:0 38px 85px #000b;backdrop-filter:blur(8px)}.hp-stage-top{display:flex;justify-content:space-between;align-items:center;padding-bottom:1rem;border-bottom:1px solid #334139}.hp-stage-top small,.hp-stage-top b,.hp-stage-card small{color:var(--lime);font:850 .68rem var(--agate);letter-spacing:.11em}.hp-stage-main h2{max-width:440px;margin:1.5rem 0;font:800 clamp(2rem,3.3vw,3.25rem)/.95 var(--agate);letter-spacing:-.04em}.hp-stage-rows{border:1px solid #2d3933}.hp-stage-rows a{display:grid;grid-template-columns:40px 1fr auto;gap:1rem;align-items:center;min-height:62px;padding:0 1rem;border-bottom:1px solid #2d3933;color:#f4f7f3;text-decoration:none;font:800 .9rem var(--agate);text-transform:uppercase}.hp-stage-rows a:last-child{border-bottom:0}.hp-stage-rows a:hover,.hp-stage-rows a:focus-visible{background:#c6f53c;color:#081008;outline:none}.hp-stage-rows i{color:#7f8b83;font-style:normal}.hp-stage-card{position:absolute;z-index:2;display:flex;flex-direction:column;justify-content:flex-end;padding:1.1rem;border:1px solid #67766d;background:#121a16;color:#fff;text-decoration:none;box-shadow:0 22px 45px #0009;transition:transform .2s,border-color .2s}.hp-stage-card:hover,.hp-stage-card:focus-visible{transform:translateY(-5px);border-color:var(--lime);outline:none}.hp-stage-card strong{display:block;margin:.5rem 0 .15rem;font:900 1.35rem var(--agate);letter-spacing:-.03em}.hp-stage-card span{color:#c9d0ca;font:700 .75rem var(--agate)}.hp-stage-college{right:0;top:0;width:150px;height:132px;background:linear-gradient(145deg,#19261f,#0d1310)}.hp-stage-college strong{color:var(--lime);font-size:2rem}.hp-stage-team{right:0;bottom:16px;width:182px;height:128px}.hp-stage-league{left:0;bottom:0;width:190px;height:112px;background:var(--lime);border-color:var(--lime);color:#091009}.hp-stage-league small,.hp-stage-league span{color:#172015}.hp-who{margin-top:1.5rem;background:#0b100de8;border-color:#465249}.hp-platform{max-width:1240px;padding-top:clamp(4rem,7vw,7rem)}.hp-bento{display:grid;grid-template-columns:1.12fr .88fr 1fr;grid-template-rows:250px 250px;gap:1rem;margin-top:2rem}.hp-bento-card{position:relative;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden;padding:1.5rem;border:1px solid #354239;background:#111814;color:#fff;text-decoration:none;transition:transform .2s,border-color .2s}.hp-bento-card:after{content:"→";position:absolute;right:1.25rem;bottom:.9rem;color:#ffffff13;font:900 6rem var(--agate);transition:color .2s,transform .2s}.hp-bento-card:hover,.hp-bento-card:focus-visible{transform:translateY(-5px);border-color:var(--lime);outline:none}.hp-bento-card:hover:after,.hp-bento-card:focus-visible:after{color:#c6f53c26;transform:translateX(5px)}.hp-bento-card small{color:var(--lime);font:850 .7rem var(--agate);letter-spacing:.12em}.hp-bento-card h3{position:relative;z-index:1;max-width:480px;margin:.8rem 0;font:850 clamp(1.8rem,3.2vw,3rem)/.96 var(--agate);letter-spacing:-.045em}.hp-bento-card span{position:relative;z-index:1;color:#d6ddd7;font:800 .75rem var(--agate);letter-spacing:.04em;text-transform:uppercase}.hp-bento-nfl{grid-row:1/3;background:radial-gradient(circle at 90% 10%,#c6f53c21,transparent 36%),#101713}.hp-bento-nfl h3{font-size:clamp(2.4rem,4.4vw,4.6rem)}.hp-bento-college{grid-column:2/4;background:radial-gradient(circle at 85% 0,#6f8f7830,transparent 38%),#111a16}.hp-bento-team{grid-column:2;background:#18221c}.hp-bento-league{grid-column:3;background:var(--lime);border-color:var(--lime);color:#091009}.hp-bento-league small,.hp-bento-league span{color:#172015}.hp-bento-league:after{color:#0a100b12}@media(max-width:1000px){.hp-hero-grid{grid-template-columns:1fr}.hp-home-copy{max-width:720px}.hp-product-stage{min-height:500px}.hp-home-hero{min-height:0}.hp-who{margin-top:2rem}}@media(max-width:780px){.hp-home-copy h1{font-size:clamp(3.7rem,16vw,5.4rem)}.hp-product-stage{min-height:470px}.hp-stage-main{inset:26px 0 64px}.hp-stage-college{right:-4px}.hp-stage-team{right:-4px}.hp-stage-league{left:-4px}.hp-bento{grid-template-columns:1fr;grid-template-rows:auto}.hp-bento-card,.hp-bento-nfl,.hp-bento-college,.hp-bento-team,.hp-bento-league{grid-column:1;grid-row:auto;min-height:220px}.hp-bento-nfl{min-height:340px}}@media(max-width:520px){.hp-wordmark-field{display:none}.hp-product-stage{min-height:530px}.hp-stage-main{inset:10px 0 155px;padding:1rem}.hp-stage-main h2{font-size:1.8rem}.hp-stage-college{top:auto;right:auto;left:0;bottom:46px;width:38%;height:100px}.hp-stage-team{right:0;bottom:46px;width:46%;height:100px}.hp-stage-league{left:14%;bottom:0;width:64%;height:72px}.hp-who{padding:1rem}.hp-home-hero{padding-bottom:1.4rem}}
.hp-home-copy h1 span,.hp-platform .hp-section-head h2{display:inline-block;background:linear-gradient(95deg,#c6f53c 0,#88f5a8 48%,#f2ffe8 100%);-webkit-background-clip:text;background-clip:text;color:transparent;text-shadow:0 0 42px #c6f53c2b}.hp-icon{width:24px;height:24px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.hp-stage-rows a{grid-template-columns:30px 34px 1fr auto}.hp-stage-rows a>.hp-icon{color:var(--lime)}.hp-stage-rows a:hover>.hp-icon,.hp-stage-rows a:focus-visible>.hp-icon{color:#081008}.hp-stage-card>.hp-icon{position:absolute;left:1rem;top:1rem;width:27px;height:27px;color:var(--lime);filter:drop-shadow(0 0 9px #c6f53c77)}.hp-stage-league>.hp-icon{color:#10190e;filter:none}.hp-icon-chip{display:grid!important;place-items:center;width:48px;height:48px;flex:none;border:1px solid #66736b;background:#0a100d;color:var(--lime)!important;box-shadow:inset 0 0 18px #c6f53c0e,0 0 28px #c6f53c13;letter-spacing:0!important}.hp-icon-chip .hp-icon{width:26px;height:26px}.hp-bento-card>.hp-icon-chip{position:absolute;z-index:2;right:1.25rem;top:1.25rem}.hp-bento-card:hover>.hp-icon-chip,.hp-bento-card:focus-visible>.hp-icon-chip{border-color:var(--lime);box-shadow:inset 0 0 20px #c6f53c20,0 0 30px #c6f53c2e}.hp-bento-league>.hp-icon-chip{border-color:#182114;background:#111b0f;color:var(--lime)!important}.hp-bento-card{box-shadow:inset 0 1px #ffffff09,0 20px 55px #0003}.hp-bento-card:hover,.hp-bento-card:focus-visible{box-shadow:inset 0 1px #ffffff12,0 24px 68px #0007,0 0 30px #c6f53c12}.hp-product-stage:before{content:"";position:absolute;left:15%;top:14%;width:62%;height:62%;border-radius:50%;background:#c6f53c1c;filter:blur(70px);animation:hp-pulse 5s ease-in-out infinite}.hp-decision-tool{grid-template-columns:auto minmax(0,1fr) auto;background:radial-gradient(circle at 12% 50%,#c6f53c10,transparent 24%)}.hp-decision-tool h2{background:linear-gradient(95deg,#fff,#c6f53c);-webkit-background-clip:text;background-clip:text;color:transparent}@keyframes hp-pulse{0%,100%{opacity:.55;transform:scale(.92)}50%{opacity:1;transform:scale(1.08)}}@media(prefers-reduced-motion:reduce){.hp-product-stage:before{animation:none}}@media(max-width:780px){.hp-decision-tool{grid-template-columns:1fr}.hp-decision-tool>.hp-icon-chip{margin-bottom:-.5rem}}
...
def write_decision_pages(homepage: Path, payload: dict, source_page: str) -> tuple[Path, Path]:
    ...
    base = '''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Compare validated fantasy football projections and see what changes the pick."><link rel="canonical" href="https://lineupbeat.com/decision-room/nfl/">''' + head_style_text + '''</head><body>''' + nfl_nav + block + seo.site_footer() + '''</body></html>'''
    ...
    college_doc = '''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Compare validated 2026 College Week 1 fantasy projections and see what changes the pick."><link rel="canonical" href="https://lineupbeat.com/decision-room/college/"><title>College Week 1 Decision Room | Lineup Beat</title>''' + head_style_text + '''</head><body data-default-sport="college">''' + sport_header("college", "decision") + college_block + seo.site_footer() + '''</body></html>'''
