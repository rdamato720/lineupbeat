...
def render_home(payload: dict, college_payload: dict) -> str:
    ...
    nfl_feature = f'''<article class="hp-feature lb-feature-card" style="--c:{esc(featured_a['team_color'])}"><small>Featured decision · Week 1 · Half-PPR</small><div class="hp-feature-players"><div><img src="{esc(featured_a['photo'])}" alt="{esc(featured_a['name'])}"><img class="hp-team-mark" src="{esc(featured_a['team_logo'])}" alt=""><span>{esc(featured_a['position'])} · {esc(featured_a['team'])} · {"vs." if featured_a.get("home") else "at"} {esc(featured_a.get("opponent"))}</span><b>{esc(featured_a['name'])}</b><em>{featured_a_format['projected_points']:.1f} pts</em></div><i>VS</i><div><img src="{esc(featured_b['photo'])}" alt="{esc(featured_b['name'])}"><img class="hp-team-mark" src="{esc(featured_b['team_logo'])}" alt=""><span>{esc(featured_b['position'])} · {esc(featured_b['team'])} · {"vs." if featured_b.get("home") else "at"} {esc(featured_b.get("opponent"))}</span><b>{esc(featured_b['name'])}</b><em>{featured_b_format['projected_points']:.1f} pts</em></div></div><h2>Tony Pollard vs. Rico Dowdle</h2><p><b>{feature_gap:.1f}</b>-point Week 1 projection gap</p><div class="hp-boundary"><strong>What changes the call?</strong> A projection gap is not a start/sit recommendation. Current availability and validated market context can change the decision.</div><a class="hp-card-cta" href="{esc(room_url(featured_a, featured_b))}">Compare the full evidence →</a></article>'''
    ...
    nfl_body = f'''<div class="hp-experience" data-home-sport="nfl"><section class="lb-hero lb-decision-hero"><div class="lb-hero-inner"><div class="lb-hero-copy"><div class="lb-eyebrow">FANTASY FOOTBALL INTELLIGENCE</div><h1 class="lb-hero-title">Make the call with the <span>evidence in front of you.</span></h1><p class="lb-hero-description">LineupBeat brings fantasy football projections, expected opportunity, opponent context, scoring format, and clearly labeled evidence limits into one decision.</p><div class="lb-hero-actions"><a class="lb-btn lb-btn-primary" href="{NFL_ROOM_PATH}">Open the Decision Room <b>→</b></a><a class="lb-btn lb-btn-secondary" href="/nfl/rankings/">Explore 2026 Rankings</a></div><div class="lb-proof-row" aria-label="LineupBeat product standards"><div class="lb-proof"><strong>NFL + College</strong><span>Two fantasy experiences</span></div><div class="lb-proof"><strong>3 formats</strong><span>PPR, Half, Non-PPR</span></div><div class="lb-proof"><strong>Local</strong><span>Roster privacy</span></div><div class="lb-proof"><strong>Visible</strong><span>Evidence limits</span></div></div></div>{nfl_feature}</div></section>
    <section class="hp-section"><div class="hp-section-head"><small>WHAT LINEUPBEAT OFFERS</small><h2>Fantasy football tools for the decisions that matter.</h2><p>Move from a lineup question to the projection, context, and uncertainty behind it—without hiding the limits of the evidence.</p></div><div class="hp-action-grid">{action('/my-team/','MY','My Team','Connect an ESPN roster locally for private starter and bench analysis.')}{action(NFL_ROOM_PATH,'±','Decision Room','Compare two players and see what could flip the call.')}{action('/nfl/rankings/','#','Rankings','Browse overall and positional ranks in three scoring formats.')}{action('/nfl/projections/','Σ','Projections','Inspect every modeled component behind the full stat line.')}{action('/nfl/data/','⌁','NFL Fantasy Data','Explore ADP, draft value, team context, and advanced tools.')}</div></section>
...
document.addEventListener("click",function(e){var a=e.target.closest&&e.target.closest(".sport-pill");if(!a)return;e.preventDefault();setSport(a.textContent.trim().toLowerCase(),true)});window.addEventListener("popstate",function(){setSport(new URLSearchParams(location.search).get("sport")||"nfl",false)});setSport(new URLSearchParams(location.search).get("sport")||"nfl",false)})();</script>'''
    return f'''{START}{nfl_header}<main id="lineup-beat-home" class="hp-shell">{nfl_body}{college_body}{brand_body}</main>{switch_js}{END}'''
...
@media(max-width:900px){.hp-action-grid,.hp-signal-grid{grid-template-columns:1fr 1fr}.hp-call-grid,.hp-method-grid{grid-template-columns:1fr}.hp-call-art>.hp-identity{grid-template-columns:60px 1fr}.hp-call-art>.hp-identity .hp-id-art{width:60px}.hp-trust{grid-template-columns:1fr}}
