...
def render_home(payload: dict, college_payload: dict) -> str:
    ...
    college_feature = next(
        r for r in college_payload["strongest_edges"] if r["confidence"] == "Lean")
    ...
    nfl_feature = f'''<article class="hp-feature lb-feature-card hp-nfl-feature" style="--c:{esc(featured_a['team_color'])}"><small>NFL decision · Week 1 · Half-PPR</small><div class="hp-feature-players"><div><img src="{esc(featured_a['photo'])}" alt="{esc(featured_a['name'])}"><img class="hp-team-mark" src="{esc(featured_a['team_logo'])}" alt=""><span>{esc(featured_a['position'])} · {esc(featured_a['team'])} · {"vs." if featured_a.get("home") else "at"} {esc(featured_a.get("opponent"))}</span><b>{esc(featured_a['name'])}</b><em>{featured_a_format['projected_points']:.1f} pts</em></div><i>VS</i><div><img src="{esc(featured_b['photo'])}" alt="{esc(featured_b['name'])}"><img class="hp-team-mark" src="{esc(featured_b['team_logo'])}" alt=""><span>{esc(featured_b['position'])} · {esc(featured_b['team'])} · {"vs." if featured_b.get("home") else "at"} {esc(featured_b.get("opponent"))}</span><b>{esc(featured_b['name'])}</b><em>{featured_b_format['projected_points']:.1f} pts</em></div></div><h2>Tony Pollard vs. Rico Dowdle</h2><p><b>{feature_gap:.1f}</b>-point Week 1 projection gap</p><div class="hp-boundary"><strong>What changes the call?</strong> A projection gap is not a start/sit recommendation. Current availability and validated market context can change the decision.</div><a class="hp-card-cta" href="{esc(room_url(featured_a, featured_b))}">Compare the NFL evidence →</a></article>'''
    college_feature_html = f'''<article class="hp-feature hp-college-feature lb-feature-card" style="--c:{esc(cw['team_color'])}"><small>College decision · Week 1 · Yahoo scoring</small><div class="hp-feature-players"><div><img src="{esc(cw['team_logo'])}" alt="{esc(cw['team'])}"><span>{esc(cw['position'])} · {esc(cw['team'])}</span><b>{esc(cw['name'])}</b><em>{cf['projected_points']:.1f} pts</em></div><i>VS</i><div><img src="{esc(cr['team_logo'])}" alt="{esc(cr['team'])}"><span>{esc(cr['position'])} · {esc(cr['team'])}</span><b>{esc(cr['name'])}</b><em>{crf['projected_points']:.1f} pts</em></div></div><h2>{esc(cw['name'])} vs. {esc(cr['name'])}</h2><p><b>+{college_feature['gap']:.1f}</b> projection edge: {esc(cw['name'])}</p><div class="hp-boundary"><strong>What changes the call?</strong> {esc(cr['name'])} needs +{college_feature['gap'] + .1:.1f} projected points to move ahead.</div><a class="hp-card-cta" href="{COLLEGE_ROOM_PATH}?a={esc(cw['id'])}&amp;b={esc(cr['id'])}">Compare the College evidence →</a></article>'''

    ambient = '''<div class="hp-ambient-data" aria-hidden="true"><div class="hp-ambient-card hp-ambient-trend"><span>PROJECTION GAP</span><svg viewBox="0 0 210 82" focusable="false"><polyline points="5,67 28,49 49,56 70,31 91,43 115,18 139,36 162,14 184,25 205,9"/></svg></div><div class="hp-ambient-card hp-ambient-formats"><span>SCORING FORMATS</span><ol><li>PPR <b>01</b></li><li>HALF-PPR <b>02</b></li><li>NON-PPR <b>03</b></li></ol></div><div class="hp-ambient-card hp-ambient-share"><span>OPPORTUNITY SHARE</span><div class="hp-ambient-ring"><b>63%</b></div></div><div class="hp-ambient-card hp-ambient-status"><span>AVAILABILITY</span><p><b>Q</b> QUESTIONABLE</p><p><b>D</b> DOUBTFUL</p><p><b>O</b> OUT</p></div><div class="hp-ambient-card hp-ambient-volume"><span>MODELED VOLUME</span><i style="--w:82%"></i><i style="--w:66%"></i><i style="--w:54%"></i><i style="--w:38%"></i></div><div class="hp-ambient-card hp-ambient-market"><span>MARKET CONTEXT</span><ol><li>SPREAD <b>—</b></li><li>TOTAL <b>—</b></li><li>PROPS <b>—</b></li></ol></div></div>'''
    ...
    sport_body = f'''<section class="hp-section hp-sports"><div class="hp-section-head"><small>CHOOSE YOUR GAME</small><h2>NFL and College have their own dedicated experiences.</h2><p>The homepage introduces LineupBeat. Each sport keeps its own projections, scoring, rankings, player pool, and Decision Room.</p></div><div class="hp-sport-grid"><a class="hp-sport-card" href="{NFL_ROOM_PATH}"><small>NFL FANTASY FOOTBALL</small><h3>Weekly decisions and full-season research.</h3><span>Compare players in PPR, Half-PPR, and Non-PPR, then inspect rankings, projections, roster analysis, and fantasy data.</span><b>Enter NFL →</b></a><a class="hp-sport-card hp-college" href="{COLLEGE_ROOM_PATH}"><small>COLLEGE FANTASY FOOTBALL</small><h3>Week 1 decisions across 64 teams.</h3><span>Compare 2,205 players using validated Yahoo scoring, then explore dedicated Week 1 and season projections.</span><b>Enter College →</b></a></div></section>'''
    tools = f'''<section class="hp-section" id="tools"><div class="hp-section-head"><small>WHAT LINEUPBEAT OFFERS</small><h2>Fantasy football tools for the decisions that matter.</h2><p>Move from a lineup question to the projection, context, and uncertainty behind it—without hiding the limits of the evidence.</p></div><div class="hp-action-grid">{action('/my-team/','MY','My Team','Connect an ESPN roster locally for private NFL starter and bench analysis.')}{action('/nfl/rankings/','#','NFL Rankings','Browse overall and positional ranks in three scoring formats.')}{action('/nfl/projections/','Σ','NFL Projections','Inspect every modeled component behind the full stat line.')}{action('/college-fantasy-football/week-1/','C#','College Rankings','Browse Week 1 rankings across 64 modeled teams.')}{action('/college-fantasy-football/projections/','CΣ','College Projections','Explore the separate 2,351-player season dataset.')}{action('/nfl/data/','⌁','NFL Fantasy Data','Explore ADP, draft value, team context, and advanced tools.')}</div></section>'''
