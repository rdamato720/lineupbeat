...
var nav={nfl:[['Decision','/decision-room/nfl/'],['Rankings','/nfl/rankings/'],['Projections','/nfl/projections/'],['Fantasy Data','/nfl/data/']],college:[['Decision','/decision-room/college/'],['Week 1 Rankings','/college-fantasy-football/week-1/'],['Season Projections','/college-fantasy-football/projections/']]};
...
function rankText(a,b,k){let af=fmt(a,k),bf=fmt(b,k),overall=af.overall_rank===bf.overall_rank?'Overall ranks are tied.':`${af.overall_rank<bf.overall_rank?safe(a.name):safe(b.name)} ranks ${Math.abs(af.overall_rank-bf.overall_rank)} overall spots higher.`,position=af.position_rank===bf.position_rank?'Position ranks are tied.':`${af.position_rank<bf.position_rank?safe(a.name):safe(b.name)} ranks ${Math.abs(af.position_rank-bf.position_rank)} ${safe(a.position===b.position?a.position:'position')} spots higher.`;return overall+' '+position}
function marketText(a,b,k){if(a.adp==null||b.adp==null)return'Unavailable — both players need validated ADP.';let early=Number(a.adp)<Number(b.adp)?a:b,diff=Math.abs(Number(a.adp)-Number(b.adp)).toFixed(1),av=Number(a.adp)-fmt(a,k).overall_rank,bv=Number(b.adp)-fmt(b,k).overall_rank;return `Market ADP selects ${safe(early.name)} ${diff} picks earlier. Projection-rank value margin: ${safe(a.name)} ${av>=0?'+':''}${av.toFixed(1)}; ${safe(b.name)} ${bv>=0?'+':''}${bv.toFixed(1)}.`}
function formatText(a,b,k){let base=projectedLeader(a,b,k),changes=FM.filter(x=>x!==k).filter(x=>{let g=Math.abs(points(a,x)-points(b,x)),c=classification(g,Math.max(points(a,x),points(b,x))),w=projectedLeader(a,b,x);return c==='Toss-Up'||(base&&w&&w.id!==base.id)});return changes.length?`Call changes in ${changes.map(x=>L[x]).join(' / ')}.`:'The projection order holds across PPR, Half-PPR, and Non-PPR.'}
...
function caseFor(p,o,k,e){let facts=[],pf=fmt(p,k),of=fmt(o,k);if(points(p,k)>points(o,k))facts.push(`Projects ${num(points(p,k)-points(o,k))} season points higher.`);if(pf.overall_rank<of.overall_rank)facts.push(`Ranks ${of.overall_rank-pf.overall_rank} overall spots higher.`);if(p.adp!=null&&o.adp!=null&&Number(p.adp)>Number(o.adp))facts.push(`Available ${Math.abs(Number(p.adp)-Number(o.adp)).toFixed(1)} picks later by ADP.`);let ph=history(p,k),oh=history(o,k);if(ph&&oh&&Number(ph.consistency_score)>Number(oh.consistency_score))facts.push(`Higher validated 2025 consistency score (${ph.consistency_score} vs. ${oh.consistency_score}).`);if(e&&e.subject_id===p.id&&e.preferred_over_id===o.id)facts.push(`The dated August 18 editorial opinion preferred ${safe(p.name)} in this exact pair.`);return facts.length?facts.slice(0,3).map(x=>`<li>${x}</li>`).join(''):'<li>No additional validated counter-case is available.</li>'}
function quality(a,b,k,e){let cats=[['Projection','Present'],['Ranks','Present'],['ADP',a.adp!=null&&b.adp!=null?'Present':'Unavailable'],['Scoring formats','Present'],['2025 consistency',history(a,k)&&history(b,k)?'Present':'Unavailable'],['Editorial opinion',e?'Historical':'Not documented'],['Position SOS','Unavailable']],present=cats.filter(x=>['Present','Historical'].includes(x[1])).length,label=present>=6?'High':present>=4?'Moderate':'Limited';return {cats,present,label}}
function changeCards(a,b,k,lead,cls){let gap=Math.abs(points(a,k)-points(b,k)),reference=Math.max(points(a,k),points(b,k)),need=cls==='Toss-Up'?+(Math.max(2,reference*.01)+.1).toFixed(1):+(gap+.1).toFixed(1),runner=lead?(lead.id===a.id?b:a):null,flips=FM.filter(x=>x!==k).filter(x=>{let g=Math.abs(points(a,x)-points(b,x)),c=classification(g,Math.max(points(a,x),points(b,x))),w=projectedLeader(a,b,x);return c==='Toss-Up'||(lead&&w&&w.id!==lead.id)}).map(x=>L[x]),adp=a.adp!=null&&b.adp!=null?`${safe(Number(a.adp)<Number(b.adp)?a.name:b.name)} goes earlier by ${Math.abs(Number(a.adp)-Number(b.adp)).toFixed(1)} picks.`:'Unavailable — validated ADP is missing for at least one player.';return `<div class="dr-boundary-grid"><article><b>+${num(need)}</b><span>${cls==='Toss-Up'?`A displayed gap above the ${num(need-.1)}-point no-call boundary is required for a Lean.`:`${safe(runner.name)} needs this projection gain to move ahead.`}</span></article><article><b>${flips.length?safe(flips.join(' / ')):'No reversal'}</b><span>${flips.length?'These validated scoring formats remove or reverse the current projection call.':'The current order holds across all three validated formats.'}</span></article><article><b>${a.adp!=null&&b.adp!=null?'ADP order':'No ADP'}</b><span>${adp}</span></article><article><b>Unavailable</b><span>Schedule/SOS reversal needs ${safe(D.schedule_sos_required_artifact)}.</span></article></div>`}
function draw(){let a=P[A.value],b=P[B.value],k=F.value;if(!a||!b||a.id===b.id){O.innerHTML='<p class="dr-error">Choose two different players.</p>';return}let ap=points(a,k),bp=points(b,k),gap=Math.abs(ap-bp),gp=pct(gap,Math.max(ap,bp)),lead=projectedLeader(a,b,k),cls=classification(gap,Math.max(ap,bp)),w=cls==='Toss-Up'?null:lead,r=w?(w.id===a.id?b:a):null,e=opinion(a,b),q=quality(a,b,k,e),call=w?`${cls==='Lean'?'Prefer':cls==='Edge'?'Recommend':'Strongly prefer'} ${safe(w.name)}`:'No clear edge',explain=w?`${safe(w.name)} projects ${num(gap)} points (${gp.toFixed(1)}%) ahead of ${safe(r.name)}.`:`The ${num(gap)}-point difference (${gp.toFixed(1)}%) is inside the deterministic no-call band.`;
O.innerHTML=`<section class="dr-verdict"><div><small>The Call · ${cls} · ${L[k]}</small><h2>${call}</h2><p>${explain}</p></div><div class="dr-adv"><b>${gap?'+':''}${num(gap)}</b><span>season-point difference</span></div></section>${playerCards(a,b,k)}<section class="dr-evidence"><div class="dr-evidence-title"><small>Evidence stack 02</small><h2>Why</h2></div><div class="dr-why-grid"><article><h3>Projection</h3><p>${num(gap)} points · ${gp.toFixed(1)}% difference.</p></article><article><h3>Ranks</h3><p>${rankText(a,b,k)}</p></article><article><h3>Draft cost</h3><p>${marketText(a,b,k)}</p></article><article><h3>Scoring sensitivity</h3><p>${formatText(a,b,k)}</p></article><article><h3>Prior-year consistency</h3><p>${historyText(a,b,k)}</p></article><article><h3>Lineup Beat opinion</h3><p>${editorialText(a,b)}</p></article></div></section><section class="dr-cases"><div class="dr-evidence-title"><small>Balanced evidence 03</small><h2>Case for each player</h2></div><div class="dr-case-grid"><article><h3>${safe(a.name)}</h3><ul>${caseFor(a,b,k,e)}</ul></article><article><h3>${safe(b.name)}</h3><ul>${caseFor(b,a,k,e)}</ul></article></div></section><section class="dr-boundary"><div class="dr-boundary-title"><small>Decision boundaries 04</small><h2>What changes the call</h2></div>${changeCards(a,b,k,lead,cls)}</section><section class="dr-quality"><div class="dr-evidence-title"><small>Transparency 05</small><h2>Confidence and data quality</h2><p><b>${q.label}</b> · ${q.present} of ${q.cats.length} evidence categories available for this pair. Classification describes projection-edge size, not probability.</p></div><div class="dr-quality-grid">${q.cats.map(x=>`<span class="${x[1].toLowerCase().replace(' ','-')}"><b>${x[0]}</b>${x[1]}</span>`).join('')}</div><p class="dr-stamp">Projection ${safe(D.sources.projections.updated_at)} · Ranks ${safe(D.sources.ranks.updated_at)} · ADP through ${safe(D.sources.adp.updated_at)} · History: ${safe(D.sources.history.updated_at)}${e?` · Editorial ${safe(e.evidence_date)} (historical/stale)`:''}</p></section>`}
function candidates(which){let other=which===A?B:A,base=D.players.filter(p=>p.id!==other.value);if(which===B&&!X.checked&&P[A.value])base=base.filter(p=>p.position===P[A.value].position);return base}
