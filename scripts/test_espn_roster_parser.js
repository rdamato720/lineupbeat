...
async function main() {
  const fixture = parseFixture(fs.readFileSync('scripts/fixtures/espn_roster_current.html', 'utf8'));
  const rosterTable = fixture.querySelector('[aria-label="Fantasy roster"]');
  assert.equal(rosterTable.querySelectorAll('tr,[role="row"]').length, 0,
    'the sanitized current ESPN fixture must exercise the div-row regression');
  const roster = parser.requireRoster(fixture);
  assert.equal(roster.length, 9);
  assert.deepEqual(roster.map(row => row.lineupSlot), ['QB', 'RB', 'WR', 'TE', 'RB/WR/TE', 'D/ST', 'K', 'BE', 'BE']);
  ...
  for (const ignored of ['919991', '919992', '919993', '919994']) {
  ...
  assert.deepEqual(normalized.startingLineupSlots.find(row => row.slotId === 'RB/WR/TE').allowedPositions, ['RB', 'WR', 'TE']);
