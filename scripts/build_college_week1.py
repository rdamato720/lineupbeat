...
EXPECTED_SHA = "4d4acb89091b4a37f4572e68b2babb28de8f5269e81e6fdbe1506b61df9d2b0c"
...
def page(position=None):
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{e(title)} | LineupBeat</title><meta name="description" content="{e(description)}"><link rel="canonical" href="https://lineupbeat.com{path}">{seo.social_meta(title + " | LineupBeat", description, "https://lineupbeat.com" + path)}<style>{css}{seo.CRUMB_CSS}{seo.UI_CSS}{CSS}</style></head><body>{header}<main class="wwrap"><nav class="crumbs"><a href="/">Home</a><span>/</span><a href="/college-fantasy-football/projections/">College projections</a><span>/</span><b>Week 1</b></nav><header class="whero"><p class="wmeta">2026 · Week 1 · Updated {updated}</p><h1>{e(title)}</h1><p>Matchup-adjusted projections for {data["counts"]["players"]:,} players on 64 modeled teams playing from September 3–7. Rankings are built directly from each projected Yahoo-scoring stat line.</p></header><nav class="wtabs">{"".join(tabs)}</nav><div class="wtools"><input id="search" type="search" placeholder="Search players"><select id="team"><option value="">All teams</option>{"".join(f"<option>{e(t)}</option>" for t in teams)}</select></div><p class="wnote"><strong>How to use this:</strong> Points and ranks account for the opponent, private multi-book market context and expected game script. Raw lines and odds are never published. Positions come from school roster listings and may differ from fantasy-platform eligibility. Confirm late injury and depth-chart news before kickoff.</p>{content}</main>{footer}<script>(()=>{{let s=document.querySelector('#search'),t=document.querySelector('#team');function f(){{let q=s.value.toLowerCase(),tm=t.value;document.querySelectorAll('tbody tr').forEach(r=>r.hidden=!!((q&&!r.dataset.name.includes(q))||(tm&&r.dataset.team!==tm)))}}s.addEventListener('input',f);t.addEventListener('change',f)}})()</script></body></html>'''
