...
def player_payload() -> list[dict]:
    ...
    weekly = load_weekly()
    weekly_ids = {(p["name"], p["team"], p["position"]): p["id"]
    ...
    for s, p in names.items():
        ...
        p["weekly_id"] = weekly_ids.get((p["name"], p["team"], p["position"]))
...
def html(players: list[dict], built: datetime, a: dict | None = None,
         b: dict | None = None, pairs: list[tuple[dict, dict]] | None = None,
         published_path: str | None = None) -> str:
    is_pair = bool(a and b)
    if is_pair:
        rec = recommendation(a, b, "ppr")
        title = f'{a["name"]} or {b["name"]}: Who Should I Draft? | LineupBeat'
        desc = f'Compare {a["name"]} and {b["name"]} using 2026 projections, rankings, ADP, and 2025 weekly consistency, floor, and ceiling.'
        path = published_path or f'/nfl/who-should-i-draft/{a["slug"]}-vs-{b["slug"]}/'
        ...
        desc = "Advanced NFL Draft Comparison using 2026 rankings, projections, ADP, and 2025 weekly floor, ceiling, and consistency."
        path = "/nfl/who-should-i-draft/"; intro = ""
    weekly_href = "/decision-room/nfl/"
    weekly_label = "View Week 1 projections"
    if is_pair and a.get("weekly_id") and b.get("weekly_id") and a["position"] == b["position"]:
        weekly_href += "?" + urlencode({"a": a["weekly_id"], "b": b["weekly_id"], "format": "ppr"})
        weekly_label = f'Compare {a["name"]} and {b["name"]} for Week 1 (PPR)'
    weekly_link = f'<p><a id="cmpweekly" href="{base.esc(weekly_href)}">{base.esc(weekly_label)}</a></p>'
    ...
    faq = [("What does the consistency score measure?", "It combines the variation in a player's 2025 weekly fantasy scoring with the size of his games-played sample. A higher score means steadier production, not necessarily more points."),
           ("Does the tool use last year's average alone?", "No. The recommendation combines current 2026 ranking and projection with 2025 average, floor, ceiling, boom rate, bust rate, and ADP."),
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{base.esc(title)}</title><meta name="description" content="{base.esc(desc)}"><link rel="canonical" href="https://lineupbeat.com{path}">{seo.social_meta(title, desc, "https://lineupbeat.com" + path, base.OG_IMAGE)}<script type="application/ld+json">{schema}</script><style>{css}{base.PAGE_CSS}{seo.CRUMB_CSS}{seo.UI_CSS}{seo.RELATED_CSS}{CSS}</style></head><body {body_attrs}>{header}<main class="cmpwrap"><nav class="crumbs"><a href="/">Home</a><span>/</span><a href="/decision-room/nfl/">Decision Room</a><span>/</span><b>Draft Comparison</b></nav><header class="cmphead"><p class="rkeyebrow">ADVANCED DRAFT COMPARISON</p><h1>Who Should I Draft?</h1><p>This advanced comparison adds 2025 weekly floor, ceiling, consistency, and Superflex context to current projections. Season-long draft comparisons are separate from weekly projections.</p>{intro}{weekly_link}<p class="rkstatus">Updated {built:%B %d, %Y} · 2026 projections · 2025 weekly results</p></header><section class="cmpbox"><div class="cmpselectors"><div class="cmpselect"><label for="pa">Player one</label><select id="pa"></select></div><div class="versus">VS</div><div class="cmpselect"><label for="pb">Player two</label><select id="pb"></select></div></div><div class="fmt"><label for="format">Scoring format</label><select id="format"><option value="ppr">PPR</option><option value="half_ppr">Half-PPR</option><option value="non_ppr">Non-PPR</option><option value="superflex">Superflex</option></select></div><div id="cmpresult" class="cmpresult"></div></section>{f'<section class="popular"><h2>Popular draft decisions</h2><div class="pairgrid">{pair_html}</div></section>' if pair_html else ''}<section class="method"><h2>More than an average</h2><p>A season average can hide a player who alternates between 25 points and five. Lineup Beat shows the median, standard deviation, 25th-percentile floor, 75th-percentile ceiling, boom rate and bust rate from every 2025 regular-season appearance. The recommendation then balances that history against the current 2026 projection and rank.</p><p>Historical results describe what happened; they do not guarantee the same role or health.</p></section>{seo.faq_html(faq)}{seo.related_html('rankings')}</main><script id="cmpdata" type="application/json">{data}</script><script>{JS}</script>{footer}{seo.TRACKING}{seo.VIEW_CONTENT}</body></html>'''


def preserved_pairs(players):
    """Resolve public permalinks using explicit reviewed name aliases and team/position."""
    by_identity = {(p["name"], p["team"], p["position"]): p for p in players}
    result = []
    for row in json.loads((ROOT / "data/comparison_routes.json").read_text()):
        matched = []
        for identity in row["players"]:
            candidates = [by_identity[(name, identity["team"], identity["position"])]
                          for name in identity["names"]
                          if (name, identity["team"], identity["position"]) in by_identity]
            if len(candidates) > 1:
                raise ValueError(f"Ambiguous comparison identity: {row['path']}")
            matched.append(candidates[0] if candidates else None)
        result.append((row, *matched))
    return result


def unavailable_pair_html(row):
    """Keep an existing address useful without restoring excluded projections."""
    names = [p["names"][0] for p in row["players"]]
    title = f"{names[0]} or {names[1]}: Who Should I Draft?"
    desc = f"Check current fantasy football rankings and weekly projections for {names[0]} and {names[1]}."
    css, header, footer = base.site_chrome()
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{base.esc(title)} | LineupBeat</title><meta name="description" content="{base.esc(desc)}"><link rel="canonical" href="https://lineupbeat.com{row['path']}"><style>{css}{base.PAGE_CSS}{seo.CRUMB_CSS}{seo.UI_CSS}{CSS}</style></head><body>{header}<main class="cmpwrap"><header class="cmphead"><h1>{base.esc(title)}</h1><p>A current season comparison is unavailable for this pair.</p><p><a href="/nfl/week-1/projections/">Browse Week 1 projections</a> &middot; <a href="/nfl/rankings/">View season rankings</a></p></header></main>{footer}{seo.TRACKING}</body></html>'''
...
def main() -> int:
    ...
    hub_html = html(players, built, pairs=pairs).replace(
        ...
        'This advanced comparison adds 2025 weekly floor, ceiling, consistency, and Superflex context to current projections. Season-long draft comparisons are separate from weekly projections.',
        f"Our {len(players)}-player draft comparison includes season rankings, projections and ADP, alongside 2025 weekly consistency, floor and ceiling. Planning your lineup? Open the weekly projections below.",
    ...
    routes = [(f'/nfl/who-should-i-draft/{a["slug"]}-vs-{b["slug"]}/', a, b)
              for a, b in pairs]
    for row, a, b in preserved_pairs(players):
        if a and b:
            routes = [item for item in routes if item[0] != row["path"]]
            routes.append((row["path"], a, b))
        else:
            dest = SITE / row["path"].strip("/") / "index.html"
            dest.parent.mkdir(parents=True, exist_ok=True)
            dest.write_text(seo.check_page(unavailable_pair_html(row), str(dest)))
    for path, a, b in routes:
        dest = SITE / path.strip("/") / "index.html"
        dest.parent.mkdir(parents=True, exist_ok=True)
        related = [(x, y) for x, y in pairs
                   if (x["position"] == a["position"] and
                       {x["slug"], y["slug"]} != {a["slug"], b["slug"]})][:12]
        pair_html = html(players, built, a, b, related, published_path=path).replace(
            '<meta name="viewport"', font_links + '<meta name="viewport"', 1
        ).replace(
            "<h1>Who Should I Draft?</h1>",
    ...
    print(f"  wrote {len(list(OUT.glob('*/index.html'))) + 1} pages under {OUT.relative_to(ROOT)}")
    return 0
