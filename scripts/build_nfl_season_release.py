...
const rows=[...document.querySelectorAll('tr[data-player-id]')],showAll=document.querySelector('#season-show-all');
function update(){const q=search.value.trim().toLowerCase(),p=position.value,t=team.value,f=format.value,capped=document.body.dataset.top&&!q&&!p&&!t;let count=0;
rows.sort((a,b)=>Number(a.dataset[f+'Rank'])-Number(b.dataset[f+'Rank'])).forEach(r=>{r.parentElement.append(r);r.hidden=!(r.dataset.name.includes(q)&&(!p||r.dataset.position===p)&&(!t||r.dataset.team===t)&&(!capped||Number(r.dataset[f+'Rank'])<=200));if(!r.hidden)count++;r.querySelector('.rank').textContent=r.dataset[(document.body.dataset.position?'position_':'')+f+'Rank'];r.querySelector('.pointvalue').textContent=Number(r.dataset[f+'Points']).toFixed(1);r.querySelector('.pointlabel').textContent=format.selectedOptions[0].text;});
...
if(showAll)showAll.addEventListener('click',()=>{delete document.body.dataset.top;showAll.hidden=true;update();});
'''
...
def wrapper(title,path,body,position='',script='',section='projections'):
...
def build():
    ...
    for pos in [None,'QB','RB','WR','TE']:
        ...
        write(path,board_page(model,ranking,path,pos=pos,top=pos is None))
