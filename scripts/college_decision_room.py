...
<main id="college-decision-room" class="dr-shell cdr" hidden aria-busy="true">
  <section class="dr-hero">
    ...
    <section class="dr-compare" aria-labelledby="cdr-compare-title"><div class="dr-console-body">
...
function statusHtml(p){let a=p.availability||{};return a.source?`<a href="${safe(a.source)}" target="_blank" rel="noopener">${safe(status(p))}</a> · ${dateLabel(a.reported_at)}`:safe(status(p))}
...
<section class="dr-quality"><div class="dr-evidence-title"><small>Sources</small><h2>Available information</h2><p><b>How the numbers compare</b> · ${state}. These comparisons do not guarantee a result.</p></div><div class="dr-quality-grid">${[['Projection','Present'],['Ranks','Present'],['Game market',am.state==='available'&&bm.state==='available'?'Present':'Partial'],['Player market',playerEvidence.length===2?'Present':playerEvidence.length?'Partial':'Unavailable'],['Opportunity','Present'],['Opponent','Present'],['ADP','Unavailable'],['History','Unavailable'],['Availability',conditional?'Check status':'Reported']].map(x=>`<span class="${x[1].toLowerCase().replace(' ','-')}"><b>${x[0]}</b>${x[1]}</span>`).join('')}</div><p class="dr-stamp">Projections updated ${dateLabel(D.updated_at)} · Game lines available for 57 of 65 teams. Check player availability before kickoff. Projections and game lines are estimates, not guarantees.</p></section></div></details>`
}
...
document.getElementById('cdr-meta').textContent=`Week ${D.week} · Yahoo scoring · Updated ${dateLabel(D.updated_at)}`;
...
A.addEventListener('change',()=>{let a=byLabel[A.value];if(a&&!X.checked){let b=pool(B).find(p=>p.id!==a.id);if(b)B.value=label(b)}lists();draw()});B.addEventListener('change',draw);[POS,TEAM,X].forEach(x=>x.addEventListener('change',()=>{let rows=pool(A),a=byLabel[A.value];if(!a||!rows.some(p=>p.id===a.id)){A.value=rows.length?label(rows[0]):'';a=byLabel[A.value]}let choices=pool(B).filter(p=>p.id!==a?.id),b=byLabel[B.value];if(!b||!choices.some(p=>p.id===b.id))B.value=choices.length?label(choices[0]):'';lists();draw()}));document.querySelectorAll('.cdr-open').forEach(x=>x.addEventListener('click',()=>{POS.value='';TEAM.value='';A.value=label(P[x.dataset.a]);B.value=label(P[x.dataset.b]);lists();draw();document.getElementById('cdr-compare-title').scrollIntoView({behavior:'smooth'})}))
