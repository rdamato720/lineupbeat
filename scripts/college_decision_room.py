...
<main id="college-decision-room" class="dr-shell cdr" hidden aria-busy="true">
  <section class="dr-hero">
    ...
    <p class="dr-lede">Compare projection, expected opportunity, opponent, and delayed sportsbook context—then see what could change the call.</p>
    <p id="cdr-meta" class="dr-week-note">Loading the validated College Week 1 evidence…</p>
    <p class="dr-week-note">Validated scope: 2,205 players across 64 teams · Yahoo scoring only · 30-second-delayed main-line sportsbook context for all modeled teams. Sportsbook context describes the game environment; it is not a player prop or a projection input. Validated College ADP is not available. Current player availability is also unavailable.</p>
...
if(D.sport!=='college'||D.mode!=='weekly'||D.market?.state!=='available_delayed_consensus')throw Error();
...
function cards(a,b){return `<div class="dr-player-grid">${[a,b].map(p=>`<article><div class="dr-person" style="--team-accent:${safe(p.team_color)}">${crest(p)}<div><small>${safe(p.team)} · ${safe(p.position)}</small><h3>${safe(p.name)}</h3></div></div><dl><div><dt>Projected points</dt><dd>${num(shown(p))}</dd></div><div><dt>Overall rank</dt><dd>#${p.formats.yahoo.overall_rank}</dd></div><div><dt>Modeled opportunity</dt><dd>${opportunity(p)}</dd></div><div><dt>Sportsbook environment</dt><dd>${marketLine(p)}</dd></div></dl></article>`).join('')}</div>`}
...
let aligned=lead&&workload?.id===lead.id&&marketLead?.id===lead.id,state=lead?(aligned?'Aligned':'Mixed'):'Toss-Up',w=c==='Toss-Up'?null:lead,r=w?(w.id===a.id?b:a):null;
...
<article><h3>Evidence limits</h3><p>Current player availability, College ADP, player props, and additional scoring formats are unavailable.</p></article>
</div></section>
...
<section class="dr-quality"><div class="dr-evidence-title"><small>Transparency 05</small><h2>Data coverage and evidence agreement</h2><p><b>Data coverage</b> · 5 of 9 evidence categories available for this pair.</p><p><b>Evidence agreement</b> · ${state}. The label summarizes direction, not probability or certainty.</p></div><div class="dr-quality-grid">${[['Projection','Present'],['Ranks','Present'],['Sportsbook','Present'],['Opportunity','Present'],['Opponent','Present'],['ADP','Unavailable'],['Scoring formats','Unavailable'],['History','Unavailable'],['Editorial','Not documented']].map(x=>`<span class="${x[1].toLowerCase().replace(' ','-')}"><b>${x[0]}</b>${x[1]}</span>`).join('')}</div><p class="dr-stamp">Projection and ranks ${safe(D.updated_at)} · TheRundown main-line consensus captured ${safe(D.market.captured_on)} · ${D.market.data_delay_seconds}-second plan delay · Pinnacle, DraftKings, FanDuel · not player props or a projection input</p></section>`
}
