...
<section class="dr-quality"><div class="dr-evidence-title"><small>Transparency 05</small><h2>Data coverage and evidence agreement</h2><p><b>Evidence agreement</b> · ${state}. The label summarizes direction, not probability or certainty.</p></div><div class="dr-quality-grid">${[['Projection','Present'],['Ranks','Present'],['Game market','Present'],['Player market',playerEvidence.length===2?'Present':playerEvidence.length?'Partial':'Unavailable'],['Opportunity','Present'],['Opponent','Present'],['ADP','Unavailable'],['History','Unavailable'],['Availability','Unavailable']].map(x=>`<span class="${x[1].toLowerCase().replace(' ','-')}"><b>${x[0]}</b>${x[1]}</span>`).join('')}</div><p class="dr-stamp">Projection and ranks ${safe(D.updated_at)} · TheRundown markets captured ${safe(D.market.captured_on)} · 30-second-delayed market context · exact player-component markets only · a market input is not an outcome or guarantee</p></section></div></details>`
}
