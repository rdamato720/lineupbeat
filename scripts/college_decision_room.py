...
<main id="college-decision-room" class="dr-shell cdr" hidden aria-busy="true">
  <section class="dr-hero">
    ...
    <div class="dr-mode">Week 1 projections · Yahoo scoring</div>
    <h1 id="cdr-title">College Decision Room</h1>
    <p class="dr-lede">Choose two players. We’ll show the pick first and keep the supporting evidence one click away.</p>
    <p id="cdr-meta" class="dr-week-note">Loading College Week 1 projections…</p>
    <p class="dr-week-note">Week 1 · Yahoo scoring · 2,205 players · 64 teams · Current player availability is not included.</p>
  ...
  <section class="dr-section"><div class="dr-section-head"><div><small>Separation</small><h2>Strongest Projection Edges</h2></div><p>Largest adjacent same-position edges in the Week 1 rankings.</p></div><div id="cdr-edges" class="dr-card-grid"></div></section>
...
const A=document.getElementById('cdr-a'),B=document.getElementById('cdr-b'),AL=document.getElementById('cdr-a-list'),BL=document.getElementById('cdr-b-list'),POS=document.getElementById('cdr-position'),TEAM=document.getElementById('cdr-team'),X=document.getElementById('cdr-cross-position'),O=document.getElementById('cdr-result');
    const shown=p=>+Number(p.formats.yahoo.projected_points).toFixed(1),num=v=>Number(v).toFixed(1),gapText=v=>Number(v)<.1?'&lt;0.1':num(v),pct=(g,r)=>+(g/Math.max(Math.abs(r),.1)*100).toFixed(1),market=p=>D.market_context_by_team[p.team_id];
...
let call=!w?'Too close to call':`Pick: ${safe(w.name)}`;
...
O.innerHTML=`<section class="dr-verdict"><div><small>LineupBeat pick · Yahoo</small><h2>${call}</h2><p><strong>${w?projection:`No clear edge. ${projection}`}</strong>${state==='Mixed'?' Some supporting evidence points the other way.':''}</p><p class="dr-caution">Current player availability is not included.</p></div><div class="dr-adv"><b>${gap?'+':''}${num(gap)}</b><span>Week 1 point difference</span></div></section><details class="dr-full"><summary>See full comparison <span>Players, evidence, and what could change the pick</span></summary><div class="dr-full-body">${cards(a,b)}
...
<article><h3>Evidence limits</h3><p>Player-market coverage varies. College ADP is not available. Current player availability and additional scoring formats are unavailable.</p></article>
</div></section>
...
<section class="dr-quality"><div class="dr-evidence-title"><small>Sources</small><h2>Data coverage and evidence agreement</h2><p><b>Evidence agreement</b> · ${state}. The label summarizes direction, not probability or certainty.</p></div><div class="dr-quality-grid">${[['Projection','Present'],['Ranks','Present'],['Game market','Present'],['Player market',playerEvidence.length===2?'Present':playerEvidence.length?'Partial':'Unavailable'],['Opportunity','Present'],['Opponent','Present'],['ADP','Unavailable'],['History','Unavailable'],['Availability','Unavailable']].map(x=>`<span class="${x[1].toLowerCase().replace(' ','-')}"><b>${x[0]}</b>${x[1]}</span>`).join('')}</div><p class="dr-stamp">Projection and ranks ${safe(D.updated_at)} · TheRundown markets captured ${safe(D.market.captured_on)} · 30-second-delayed market context · exact player-component markets for 112 players · a market input is not an outcome or guarantee</p></section></div></details>`
}
...
}).catch(()=>{root.hidden=false;root.setAttribute('aria-busy','false');document.getElementById('cdr-meta').textContent='College Week 1 projections could not be loaded. Please try again.'})
})();'''
