...
Football rosters locally for My Team. ESPN also supports importing available
league seasons for commissioner review in League History. It does not request
...
1. Download the submission artifact from the successful development workflow,
   or download version 0.4.0 from the support page, and unzip the
   store package.
...
Version 0.4.0 adds Yahoo and CBS visible-roster capture while preserving the
bounded ESPN history import, local snapshot storage, incomplete-season reporting,
and commissioner identity review. The ESPN history importer handles up to 25 seasons and captures league/team
