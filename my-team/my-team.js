(function(){
  ...
  function decisionCard(row,format){
    ...
    const reason=`${bench.name} projects ${gap} points ahead in your ${slot} slot.`;
    const benchUsage=opportunity(bench),starterUsage=opportunity(starter);
    return `<article class="mt-decision"><div class="mt-decision-head"><div><small>${escape(row.classification)} · ${escape(labels[format])}</small><h3>Start ${escape(bench.name)}</h3><p>Over ${escape(starter.name)} · ${escape(reason)}</p></div><div class="mt-decision-score"><strong>+${gap}</strong><span>point edge</span></div></div><div class="mt-versus"><div><img src="${escape(bench.photo||bench.teamLogo)}" alt=""><b>${escape(bench.name)}</b><span>Bench · ${row.benchPoints.toFixed(1)} pts ${statusBadge(row.bench,bench)}</span></div><i>over</i><div><img src="${escape(starter.photo||starter.teamLogo)}" alt=""><b>${escape(starter.name)}</b><span>Starter · ${row.starterPoints.toFixed(1)} pts ${statusBadge(row.starter,starter)}</span></div></div><div class="mt-decision-facts"><p><b>Matchups</b>${escape(bench.name)} ${escape(opponent(bench))}; ${escape(starter.name)} ${escape(opponent(starter))}.</p>${benchUsage||starterUsage?`<p><b>Opportunity</b>${escape(bench.name)}: ${escape(benchUsage||'unavailable')}. ${escape(starter.name)}: ${escape(starterUsage||'unavailable')}.</p>`:''}<p><b>Availability</b>${escape(availability(bench))} ${escape(availability(starter))}</p></div><a class="mt-player-link mt-compare" href="${escape(comparisonUrl(bench,starter,format))}">Compare players →</a></article>`;
  }
